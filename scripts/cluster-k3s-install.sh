...
INSTALL_K3S_VERSION="v1.31.3+k3s1" K3S_KUBECONFIG_MODE="644" INSTALL_K3S_SKIP_DOWNLOAD=true INSTALL_K3S_EXEC="server --server https://$DOMAIN:6443 --disable traefik --embedded-registry --etcd-expose-metrics  --prefer-bundled-bin --node-name crucible-ctrl-02 --tls-san ${DOMAIN:-crucible.io} --token-file /home/$SUDO_USERNAME/node-token" /home/$SUDO_USERNAME/k3s-install.sh
mkdir ~/.kube 
...
govc guest.upload -vm crucible-ctrl-02 -l $SUDO_USERNAME:$SUDO_PASSWORD -f ./cmds.sh /home/$SUDO_USERNAME/registries.yaml
...
#     govc guest.upload -vm $node -l $SUDO_USERNAME:$SUDO_PASSWORD ./dist/generic/k3s-install.sh ~/k3s-install.sh
#     govc guest.upload -vm $node -l $SUDO_USERNAME:$SUDO_PASSWORD ./dist/generic/k3s ~/k3s
#     govc guest.upload -vm $node -l $SUDO_USERNAME:$SUDO_PASSWORD ./dist/ssl/server/tls/node-token ~/node-token
#     govc guest.run -vm $node -l $SUDO_USERNAME:$SUDO_PASSWORD -e SUDO_PASSWORD=$SUDO_PASSWORD \
#         echo "$SUDO_PASSWORD" | sudo -E -S NONINTERACTIVE=1 apt-get install -y build-essential jq nfs-common sshpass postgresql-client make logrotate git unzip apache2-utils && \
...
#         INSTALL_K3S_VERSION="v1.31.3+k3s1" K3S_KUBECONFIG_MODE="644" INSTALL_K3S_SKIP_DOWNLOAD=true INSTALL_K3S_EXEC="server --server https://$DOMAIN:6443 --disable traefik --embedded-registry --etcd-expose-metrics  --prefer-bundled-bin --node-name $node --tls-san ${DOMAIN:-crucible.io} --token-file ~/node-token" ~/k3s-install.sh && \
#         mkdir ~/.kube && \
#         sudo cp /etc/rancher/k3s/k3s.yaml ~/.kube/config && \
#         sed -i "s/default/crucible-appliance/g" ~/.kube/config && \
#         sed -i "s/127.0.0.1/${DOMAIN}/g" ~/.kube/config && \
