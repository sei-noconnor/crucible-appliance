...
INSTALL_K3S_EXEC="server --server https://${DOMAIN:-onprem.phl-imcite.net}:6443 --disable traefik \
--embedded-registry --etcd-expose-metrics  --prefer-bundled-bin --tls-san ${DOMAIN:-onprem.phl-imcite.net} \
...
INSTALL_K3S_VERSION="v1.31.3+k3s1" K3S_KUBECONFIG_MODE="644" INSTALL_K3S_SKIP_DOWNLOAD=true \
INSTALL_K3S_EXEC="agent --server https://${DOMAIN:-onprem.phl-imcite.net}:6443 --disable traefik \
--embedded-registry --etcd-expose-metrics  --prefer-bundled-bin --tls-san ${DOMAIN:-onprem.phl-imcite.net} \
--token-file /home/crucible/node-token" /home/crucible/k3s-install.sh
sudo cp /etc/rancher/k3s/k3s.yaml ~/.kube/config
sed -i "s/default/crucible-appliance/g" ~/.kube/config
