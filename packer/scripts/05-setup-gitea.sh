...
timeout 60s bash -c 'while [[ "$(curl -s -o /dev/null -w ''%{http_code}'' https://onprem.twn-imcite.net/gitea)" != "200" ]]; do sleep 5; done'
...
# Create Organization
curl "${CURL_OPTS[@]}" \
  --request POST "https://onprem.twn-imcite.net/gitea/api/v1/orgs" \
  --data @- <<EOF
  {
...
# Create repo
curl "${CURL_OPTS[@]}" \
    --request POST "https://onprem.twn-imcite.net/gitea/api/v1/orgs/${GITEA_ORG}/repos" \
    --data @- <<EOF
{
  "auto_init": true,
  "default_branch": "",
...
find $REPO_DEST -name "Application.yaml" -exec sed -i "s/file:\/\/\/crucible-repo\/crucible-appliance/https:\/\/onprem.twn-imcite.net\/gitea\/${GITEA_ORG}\/crucible-appliance.git/g" {} \;
find $REPO_DEST -name "*.yaml" -exec sed -i "s/https:\/\/github.com\/sei-noconnor/https:\/\/onprem.twn-imcite.net\/gitea\/${GITEA_ORG}/g" {} \;
...
git -C $REPO_DEST remote add appliance https://administrator:${ADMIN_PASS}@onprem.twn-imcite.net/gitea/${GITEA_ORG}/crucible-appliance.git
