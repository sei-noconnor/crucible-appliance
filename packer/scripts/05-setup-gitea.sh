...
timeout 60s bash -c 'while [[ "$(curl -s -o /dev/null -w ''%{http_code}'' https://onprem.phl-imcite.net/gitea)" != "200" ]]; do sleep 5; done'
...
# Create Organization
curl "${CURL_OPTS[@]}" \
  --request POST "https://onprem.phl-imcite.net/gitea/api/v1/orgs" \
  --data @- <<EOF
  {
...
# Create repo
curl "${CURL_OPTS[@]}" \
    --request POST "https://onprem.phl-imcite.net/gitea/api/v1/orgs/${GITEA_ORG}/repos" \
    --data @- <<EOF
{
  "auto_init": true,
  "default_branch": "",
...
git -C $REPO_DEST remote add appliance https://administrator:${ADMIN_PASS}@onprem.phl-imcite.net/gitea/${GITEA_ORG}/crucible-appliance.git
